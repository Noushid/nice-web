...
class Blog_Controller extends Check_Logged
{
    ...
    function store()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if (!empty($uploaded)) {
                ...
                $resize_error = [];
                if ($file_id and $this->blog->add($post_data)) {
                    /*****Create Thumb Image****/
    ...
    function delete_image($id)
    {
        ...
        $blog = $this->blog->select_where(['id' => $id]);

        if ($this->file->remove($blog[0]->file_id)) {
            $data['file_id'] = null;
            if ($this->blog->edit($data,$id)) {
                if (file_exists(getwdir() . 'uploads/' . $blog[0]->file_name)) {
                    unlink(getwdir() . 'uploads/' . $blog[0]->file_name);
                }
    ...
    public function delete($id)
    {
        ...
        if ($this->blog->remove($id)) {
            if ($this->file->remove($blog[0]->file_id) && file_exists(getwdir() . 'uploads/' . $blog[0]->file_name)) {
                unlink(getwdir() . 'uploads/' . $blog[0]->file_name);
                $this->output->set_content_type('application/json')->set_output(json_encode(['msg' => 'Blog Deleted']));
            } else {
                $this->output->set_content_type('application/json')->set_output(json_encode(['msg' => 'Blog Deleted and file delete error']));
            }
        } else {
            $this->output->set_status_header(500, 'Server Down');
